...
export default function PackageForm({ packageType, packageId }) {
  ...
  const addPackageEntry = () => {
    setPackagesAndCars((prev) => [
      ...prev,
      { id: generateUniqueId(), packageName: "", cars: [] },
    ]);
    setIsDirty(true);
  };
  ...
  const updatePackageName = (packageIdToUpdate, newName) => {
    setPackagesAndCars((prev) =>
      prev.map((pkg) =>
        pkg.id === packageIdToUpdate ? { ...pkg, packageName: newName } : pkg
      )
    );
    setIsDirty(true);
  };
  ...
  const removePackageEntry = (packageIdToRemove) => {
    setPackagesAndCars((prev) =>
      prev.filter((pkg) => pkg.id !== packageIdToRemove)
    );
    setIsDirty(true);
  };
  ...
  const removeCarFromPackage = (packageIdToUpdate, carIdToRemove) => {
    setPackagesAndCars((prev) =>
      prev.map((pkg) =>
        pkg.id === packageIdToUpdate
          ? { ...pkg, cars: pkg.cars.filter((car) => car.id !== carIdToRemove) }
          : pkg
      )
    );
    setIsDirty(true);
  ...
  const addSightseeingPlace = () => {
    setSightseeingPlaces((prev) => [
      ...prev,
      { id: generateUniqueId(), text: "", imageUrl: "", imageFile: null },
    ]);
    setIsDirty(true);
  };
  ...
  const handleSightseeingPlaceImageFileChange = (placeId, file) => {
    setSightseeingPlaces((prev) =>
      prev.map((place) =>
        place.id === placeId ? { ...place, imageFile: file } : place
      )
    );
    setIsDirty(true);
  };
  ...
  const handleFemaleDressCodeFileChange = (e) => {
    if (e.target.files) {
      setNewFemaleDressCodeFiles((prev) => [
        ...prev,
        ...Array.from(e.target.files),
      ]);
      setIsDirty(true);
  ...
  const handleSectionImageFileChange = (sectionId, file) => {
    setSections((prev) =>
      prev.map((section) =>
        section.id === sectionId ? { ...section, imageFile: file } : section
      )
    );
    setIsDirty(true);
  };
    ...
    setSections((prev) =>
      prev.map((section) =>
        section.id === sectionId
          ? { ...section, imageUrl: "", imageFile: null, hasImage: false }
          : section
      )
    );
    setIsDirty(true);
  ...
  const addFaq = () => {
    setFaqs((prev) => [
      ...prev,
      { id: generateUniqueId(), question: "", answer: "" },
    ]);
    setIsDirty(true);
  };
  ...
  useEffect(() => {
    ...
    const handleRouteChange = (url) => {
      if (
        isDirty &&
        !confirm("Are you sure you want to move back without saving?")
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const processedSightseeingPlaces = await Promise.all(
        sightseeingPlaces.map(async (place) => {
          ...
          if (place.imageFile) {
            ...
            placeImageUrl = url;
            console.log(
              `Uploaded place image ${place.imageFile.name}, URL: ${placeImageUrl}`
            );
          }
          return {
            id: place.id,
      ...
      for (const file of newMaleDressCodeFiles) {
        const url = await uploadToMinIO(
          file,
          `${storagePathPrefix}/dress-code/male`
        );
        uploadedMaleDressCodeUrls.push(url);
      }
      const allMaleDressCodeUrls = [
        ...maleDressCodeImages,
        ...uploadedMaleDressCodeUrls,
      ];
      ...
      for (const file of newFemaleDressCodeFiles) {
        const url = await uploadToMinIO(
          file,
          `${storagePathPrefix}/dress-code/female`
        );
        uploadedFemaleDressCodeUrls.push(url);
      }
      const allFemaleDressCodeUrls = [
        ...femaleDressCodeImages,
        ...uploadedFemaleDressCodeUrls,
      ];
      ...
      <Card className="w-full max-w-3xl mx-auto">
        <CardHeader>
          <CardTitle className="text-center">
            {isEditMode
              ? `Edit ${getDisplayTitle(packageType)}`
              : `Add New ${getDisplayTitle(packageType)}`}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              <p className="text-sm text-gray-500 mt-1">
                Upload multiple images for this package.
              </p>
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("packagesAndCars", newTitle)
                }
              ...
              <div className="space-y-6 p-4 bg-gray-50 rounded-md border border-gray-200">
                {packagesAndCars.map((pkg) => (
                  <div
                    key={pkg.id}
                    className="border border-gray-300 p-4 rounded-md bg-white relative"
                  >
                    <Button
                      type="button"
                      variant="destructive"
                    ...
                    <div className="mb-4">
                      <Label htmlFor={`package-name-${pkg.id}`}>
                        Package Name
                      </Label>
                      <Input
                    ...
                    <div className="space-y-3 mb-4">
                      {pkg.cars.map((car) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <Label htmlFor={`seat-capacity-${car.id}`}>
                              Seat Capacity
                            </Label>
                            <Input
                    ...
                    <Button
                      type="button"
                      onClick={() => addCarToPackage(pkg.id)}
                      className="mt-2"
                    >
                      Add Car
                    </Button>
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("packageIncludes", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("passengerNotes", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
              ...
              <div className="space-y-4 p-4 bg-gray-50 rounded-md border border-gray-200">
                ...
                <Button
                  type="button"
                  onClick={() => addPoint(setPassengerNotes)}
                  className="mt-3"
                >
                  Add Point
                </Button>
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("dressCode", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
              ...
              <div className="space-y-4 p-4 bg-gray-50 rounded-md border border-gray-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        {newMaleDressCodeFiles.map((file, index) => (
                          <div key={index} className="relative group w-24 h-16">
                            <img
                              src={
                                URL.createObjectURL(file) || "/placeholder.webp"
                              }
                        ...
                        {maleDressCodeImages.map((url, index) => (
                          <div key={index} className="relative group w-24 h-16">
                            ...
                            <Button
                              ...
                              className="absolute top-0 right-0 h-5 w-5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                              onClick={() =>
                                removeExistingMaleDressCodeImage(url)
                              }
                            >
                              <X className="h-3 w-3" />
                              <span className="sr-only">Remove image</span>
                  ...
                  <div className="border border-gray-300 p-4 rounded-md bg-white">
                    ...
                    <Label htmlFor="female-dress-code-image">
                      Choose Image
                    </Label>
                    <Input
                        ...
                        {newFemaleDressCodeFiles.map((file, index) => (
                          <div key={index} className="relative group w-24 h-16">
                            ...
                            <Button
                              ...
                              className="absolute top-0 right-0 h-5 w-5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                              onClick={() =>
                                removeNewFemaleDressCodeImage(index)
                              }
                            >
                              <X className="h-3 w-3" />
                              <span className="sr-only">Remove image</span>
                        ...
                        {femaleDressCodeImages.map((url, index) => (
                          <div key={index} className="relative group w-24 h-16">
                            ...
                            <Button
                              ...
                              className="absolute top-0 right-0 h-5 w-5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                              onClick={() =>
                                removeExistingFemaleDressCodeImage(url)
                              }
                            >
                              <X className="h-3 w-3" />
                              <span className="sr-only">Remove image</span>
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("carPrices", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
                  ...
                  >
                    ...
                    <div className="space-y-3 mb-4 p-3 bg-gray-100 rounded-md border border-gray-200">
                      {car.prices.map((price) => (
                        ...
                        >
                          <div className="flex-1">
                            <Label htmlFor={`price-label-${price.id}`}>
                              Label
                            </Label>
                            <Input
                          ...
                          <div className="flex-1">
                            <Label htmlFor={`price-value-${price.id}`}>
                              Price
                            </Label>
                            <Input
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("sections", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
                ...
                {sections.map((section) => (
                  ...
                  >
                    ...
                    <div className="flex items-center mb-4">
                      ...
                      />
                      <Label htmlFor={`want-image-${section.id}`}>
                        Want to add/update Image?
                      </Label>
                    </div>
                    ...
                    {section.hasImage && (
                      <div className="mb-4 p-3 bg-gray-100 rounded-md border border-gray-200">
                        <Label htmlFor={`section-image-${section.id}`}>
                          Choose File
                        </Label>
                        <Input
                    ...
                    <div className="space-y-3 mb-4 p-3 bg-gray-100 rounded-md border border-gray-200">
                      {section.listInfo.map((item) => (
                        <div key={item.id} className="space-y-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              removeListInfoFromSection(section.id, item.id)
                            }
                            className="self-end"
                          >
                            Remove
                      ...
                      <Button
                        type="button"
                        onClick={() => addListInfoToSection(section.id)}
                        className="mt-2"
                      >
                        Add Info
                      </Button>
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("faq", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
              ...
              <div className="space-y-4 p-4 bg-gray-50 rounded-md border border-gray-200">
                {faqs.map((item) => (
                  <div key={item.id} className="space-y-3">
                    <div>
                      <Label htmlFor={`faq-question-${item.id}`}>
                        Question
                      </Label>
                      <RichTextEditor
                        value={item.question}
            ...
            <div>
              <EditableTitle
                ...
                onTitleChange={(newTitle) =>
                  updateSectionTitle("tables", newTitle)
                }
                placeholder="Enter section title"
                showEditIcon={true}
                required={false}
                          ...
                          <div key={headerIndex} className="flex gap-1">
                            ...
                            {table.headers.length > 1 && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  removeTableHeader(table.id, headerIndex)
                                }
                              >
                                <X className="h-3 w-3" />
                              </Button>
