...
const SEO_CONTENT_MAP = {
  "kilometer-based-car-rental-from-chennai": {
    title: "Kilometer Based Car Rental from Chennai – Flexible & Cost‑Effective Travel",
    content: [
      "Looking for a reliable <strong>kilometer based car rental from Chennai</strong> for your next trip? Our kilometer based car rental from Chennai service offers a flexible and affordable way to travel anywhere you want with complete comfort. With the kilometer based car rental from Chennai, you only pay for the kilometers you travel, making it a smart choice for day trips, airport transfers, temple tours, family outings, or business travel.",
      "Our kilometer based car rental from Chennai includes well‑maintained AC cars, experienced drivers, and transparent pricing so you can plan your journey without hidden costs. Whether you need a sedan for a small group or a larger vehicle for family travel, our kilometer based car rental from Chennai options are designed to fit your needs. You can choose the car type that suits your budget and travel plan, and our team ensures a smooth booking process.",
      "The kilometer based car rental from Chennai is perfect for visitors, locals, and frequent travelers who want flexibility and comfort without paying for unused hours. With clear billing based on distance covered, our kilometer based car rental from Chennai makes long drives and sightseeing convenient and stress‑free. Book your kilometer based car rental from Chennai today and enjoy a comfortable, safe, and memorable journey tailored to your schedule and destination."
    ]
  }
};
...
export async function generateMetadata({ params }) {
  ...
  if (slug === "kilometer-based-car-rental-from-chennai") {
    return {
      title: "Kilometer Based Car Rental from Chennai – Flexible & Cost-Effective Travel",
      description:
        "Looking for a reliable kilometer based car rental from Chennai? Pay only for the kilometers you travel. Well-maintained AC cars with professional drivers for day trips, transfers, and outstation.",
    };
  }
...
export default async function CarRentalPage({ params }) {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const localPackageSlugs = ["kilometer-based-car-rental-from-chennai"];
      ...
      if (localPackageSlugs.includes(slug)) {
        try {
          ...
          // Merge carTypes images based on id or name
          if (localData.carTypes && packageData.carTypes) {
            packageData.carTypes = packageData.carTypes.map((car) => {
              // Prioritize ID match, then fallback to name match
              const localCar =
                localData.carTypes.find((lc) => lc.id === car.id) ||
                localData.carTypes.find((lc) => lc.name === car.name);

              if (localCar) {
                // Destructure to Exclude ID from localCar to prevent overwriting the Firestore source-of-truth ID
                const { id, ...localRest } = localCar;
                return { ...car, ...localRest };
              }
              return car;
            });
          }
  ...
  if (slug === "kilometer-based-car-rental-from-chennai" && packageData) {
    console.log("FINAL Backend Response for Kilometer Based Car Rental:", JSON.stringify(packageData, null, 2));
  }
      ...
      <div className="pt-16 md:pt-20 lg:pt-24">
        ...
        <section className="bg-red-50 py-8 sm:py-12">
          <div className="container mx-auto text-center px-4">
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold text-gray-800 mb-3">
        ...
        {SEO_CONTENT_MAP[slug] && (
          <section className="py-8 sm:py-12 bg-blue-50/50">
            <div className="container mx-auto px-4">
              <div className="max-w-4xl mx-auto">
                <article className="prose prose-blue max-w-none">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-blue-900">
          ...
          <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="w-full order-1 md:order-1">
              <div className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-red-500/20 rounded-3xl transform md:rotate-6 group-hover:md:rotate-12 transition-transform duration-500"></div>
                <div className="relative bg-white rounded-3xl shadow-2xl p-4 sm:p-6 transform md:-rotate-2 group-hover:md:rotate-0 transition-transform duration-500">
                  <img
            ...
            {/* Right - Content */}
            <div className="space-y-4 bg-gray-50 p-4 sm:p-6 rounded-2xl order-2 md:order-2">
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">{packageData.title}</h2>
              {packageData.subtitle && (
                <p className="text-lg sm:text-xl text-gray-700 font-medium">{packageData.subtitle}</p>
              )}
        ...
        {packageData.carTypes && packageData.carTypes.length > 0 && (
          <section className="py-8 sm:py-12 px-4 bg-gray-50">
            <div className="container mx-auto">
              <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8">{packageData.sectionTitles?.carTypes || "Available Vehicles"}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {packageData.carTypes.map((car, index) => (
                  <Card key={car.id || index} className="overflow-hidden hover:shadow-xl transition-all duration-300 transform md:hover:scale-105 bg-white border border-gray-200 flex flex-col h-full">
                    {/* Image Section */}
                      ...
                      {(car.image || car.imageUrl) ? (
                        <div className="aspect-video relative overflow-hidden">
                          <img
                            ...
                            alt={car.name}
                            className="w-full h-full object-cover rounded-lg transition-transform duration-300 md:hover:scale-110" />
                          {/* Rating Badge */}
                          {car.rating && (
                    ...
                    <CardContent className="p-4 sm:p-6 flex flex-col flex-grow">
                      {/* Car Name */}
                      <div className="mb-4">
                        <h3 className="text-lg sm:text-xl font-bold text-gray-800 text-center">{car.name}</h3>
                      </div>
        ...
        {packageData.serviceFeatures && packageData.serviceFeatures.length > 0 && (
          <section className="py-8 sm:py-12 px-4 bg-white">
            <div className="container mx-auto">
              <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8">{packageData.sectionTitles?.serviceFeatures || "Service Features"}</h2>
        ...
        {packageData.sections && packageData.sections.length > 0 && (
          <div className="space-y-12">
            {packageData.sections.map((section, index) => (
              <section key={section.id || index} className="py-8 sm:py-12 px-4 bg-white">
                <div className="container mx-auto">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    {/* Content */}
                    <div className="space-y-4">
                      {section.contentTitle && (
                        <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">{section.contentTitle}</h2>
                      )}
                      {section.contentDescription && (
                        <div
                          className="text-gray-600 space-y-3 prose prose-sm max-w-none text-base sm:text-lg"
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {packageData.includes && packageData.includes.length > 0 && (
                <div>
                  <div className="text-center mb-6">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">{packageData.sectionTitles?.includes || "What's Included"}</h2>
              ...
              {packageData.passengerNotes && packageData.passengerNotes.length > 0 && (
                <div>
                  <div className="text-center mb-6">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">{packageData.sectionTitles?.passengerNotes || "Important Passenger Notes"}</h2>
        ...
        {packageData.termsAndConditions && packageData.termsAndConditions.length > 0 && (
          <section className="py-8 sm:py-12 px-4 bg-white">
            <div className="container mx-auto">
              <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8">{packageData.sectionTitles?.termsAndConditions || "Terms and Conditions"}</h2>
