...
export default function ManageTemplePackagesPage() {
  ...
  const fetchPackages = async () => {
    ...
    try {
      ...
      const filteredPackages = fetchedPackages.filter(
        (p) => !(p?.isDeleted || p?.deletedAt)
      );
      // Double-check sorting: sort by order field (ascending), with fallback to 999 for items without order
  ...
  const handleDelete = async (id) => {
    if (
      !confirm(
        "Are you sure you want to delete this package? This will move it to the recycle bin."
      )
    ) {
      return;
    }
          ...
          <CardContent>
            {packages.length === 0 ? (
              <p className="text-center text-gray-600 py-8">
                No temple packages found. Create one!
              </p>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {packages.map((pkg) => {
                      // Additional defensive sorting check (already sorted, but this ensures consistency)
                      return (
                        <TableRow key={pkg.id}>
                          ...
                          <TableCell className="text-right">
                            <Link
                              href={`/admin/temple-package/edit/${pkg.id}`}
                              passHref
                            >
                              <Button
                                variant="ghost"
